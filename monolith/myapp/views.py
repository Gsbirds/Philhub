...
import json
from .encoder import (
    Show_NotesDetailEncoder,
    Show_pageListEncoder,
    Show_pageDetailsDetailEncoder,
    WorksListEncoder,
    Show_ContactEncoder,
    Show_WorksDetailEncoder,
    Show_NotesListEncoder,
)
...
def show_page_details(request, id):
    if request.method == "GET":
        ...
        return JsonResponse(news, encoder=Show_pageDetailsDetailEncoder, safe=False)
...
def api_list_works(request):
    if request.method == "GET":
        ...
        return JsonResponse({"posts": posts}, encoder=WorksListEncoder)
    else:
        content = json.loads(request.body)
        try:
...
def api_show_works(request, id):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"filepath": posts.filepath.__str__(), "name": posts.name},
            encoder=Show_pageDetailsDetailEncoder,
            safe=False,
        )
    elif request.method == "DELETE":
...
def api_notes_detail(request, id):
    if request.method == "GET":
        posts = File.objects.get(id=id)
        return JsonResponse(posts, encoder=Show_NotesDetailEncoder, safe=False)
    elif request.method == "DELETE":
...
def contact(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save()
            return JsonResponse(
                form, encoder=Show_ContactEncoder, safe=False, status=200
            )
        # else:
        #     return JsonResponse(
        #         status=400
